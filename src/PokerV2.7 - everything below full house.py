...
if last_player_standing() == False:   # showdown baby
    ...
    player_hands2["fox's_hand"]['cards'] = ['7 S', '7 D', '10 C', '13 D', '13 H', '13 S', '14 C']
    ...
    def Flush():
        for name in player_hands2:
            if len(indices(list_of_suits(player_hands2[name]['cards']),'C')) >= 5 or \
               len(indices(list_of_suits(player_hands2[name]['cards']),'H')) >= 5 or \
               len(indices(list_of_suits(player_hands2[name]['cards']),'D')) >= 5 or \
               len(indices(list_of_suits(player_hands2[name]['cards']),'S')) >= 5:  # if there are 5+ cards of the same suit
                print(name, 'got a Flush')
                Flush = True
            else:
                print(name, 'didnt hit a flush.')
                Flush = False
    ...
    # High Card
    # Can now set 'five_best_cards' = the 5 flush/straight cards.!!!!!!!!!!!!!
    for name in player_hands2: # this is if all other handStr == False - take 5 highest cards out of the 7
    ...
    def HighCard(card_list = five_best_cards):
        high_card_player = [name for name in player_hands2]  # player list
        for j in range(len(card_list))):  # j = 0,1,2,3,4  # itrating through 5 highest cards
    ...
    '''
    for name in player_hands2:
        player_hands2[name]['high_card'] = sorted(card_number_list(player_hands2[name]['cards'][-5:]),reverse = True)  # last(highest) 5 cards, in reverse
